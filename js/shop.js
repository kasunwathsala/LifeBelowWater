document.addEventListener('DOMContentLoaded', function () {
  ...
  addToCartButtons.forEach((button, index) => {
    button.addEventListener('click', function () {
      ...
      const size = product.querySelector(`#size${index + 1}`)?.value || 'Standard';
      const quantity = parseInt(product.querySelector(`#quantity${index + 1}`).value);
      const color = product.querySelector('#color') ? product.querySelector('#color').value : 'N/A';
